...
% List the files that have the appropiate extension 
S = dir(fullfile(path,'*.h5'));
N=numel(S); % number of files to use
H=zeros(N);
i=30; %3, 22, 19

% Print is the file that will be analysed 
disp(strcat('This is the file ...',S(i).name)) %6 is 2000
%-------------------------------------------------------------------------

    %load the file and de groups
    %----------------------------------------------------------------------
    ...
    %Choose the resolution and threshold to use CalcSpectra
    %----------------------------------------------------------------------
    %resx=18/296; %resy=18/296; %resz=63/1024; %0.06152 
    ...
    %res=13/400;
    %threshold=96; %100; 
    % Check if by changing this the extension in the kper will be reduced
    
    %for 103
    %res=0.07;    %threshold=44;
    %----------------------------------------------------------------------
    
    %Choose the normalization before calculating the spectra
    %----------------------------------------------------------------------
    % Normalization
    ...
    % Now actually calculate the spectra
    %----------------------------------------------------------------------
    ...
    spectraB_i=CalcSpectra_CB_local(Bx/B0,By/B0,Bz/B0,res,threshold);
    clearvars Bx By Bz 
    
    spectraE_i=CalcSpectra_CB_local(Ex/E0,Ey/E0,Ez/E0,res,threshold);
    clearvars Ex Ey Ez 
    
    spectravi_i=CalcSpectra_CB_local(vix/Vai,viy/Vai,viz/Vai,res,threshold);
    clearvars vix viy viz
    
    spectrave_i=CalcSpectra_CB_local(vex/Vai,vey/Vai,vez/Vai,res,threshold);
    clearvars vex vey vez
    
    spectrani_i=CalcSpectra_CB_n(ni/n0,res,threshold);
    clearvars ni
    
    spectrane_i=CalcSpectra_CB_n(ne/n0,res,threshold);
    clearvars ne
   
    spectraJ=CalcSpectra_CB(Jx/J0,Jy/J0,Jz/J0,res,threshold);
    clearvars Jx Jy Jz
    %----------------------------------------------------------------------
    ...
    % For individual tries choose the spectra / Select the field spectra to
    % plot wether B, E, J, etc
    %----------------------------------------------------------------------
    spectraB120=spectraB_i;
    
    spectratoBp=spectraB_120;
    
    P2D=spectratoBp.P2D/max(max(spectratoBp.P2D));
    kpar=spectratoBp.kpar;
    kper=spectratoBp.kper;
    %----------------------------------------------------------------------    
    
    % Now do the P2D plot
    %----------------------------------------------------------------------
    f1=figure(91);
    [C,h]=contourf(kpar,kper,log10(P2D'),'LevelStep',0.5);
    ...
    % In case you have the scatter data from the magnetic structures this
    % is to plot the scatter data from volume clustering method on top the
    % P2D
    %----------------------------------------------------------------------
    f33=figure(33);
    [C,h]=contourf(kpar,kper,log10(P2D'),'LevelStep',0.5);
    ...
    % Now it this is for the 1D plots
    %---------------------------------------------------------------------- 
    f51=figure(51);
